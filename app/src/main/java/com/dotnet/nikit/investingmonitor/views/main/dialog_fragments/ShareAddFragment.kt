...
class ShareAddFragment(parentFragment: DaggerFragment,
    ...
    private fun addShareData() {
        ...
        onCompleteListener?.onCompleteAdding(
            Share(null, tickerEditText?.text?.toString()!!, nameEditText?.text?.toString()!!, buyPriceEditText?.text?.toString()?.toFloat()!!,
                Utils.formatStringToDate(buyDateEditText?.text?.toString())!!, CurrencyEnum.valueOf(currencySpinner?.selectedItem.toString()),
                currentPrice, AssetTypeEnum.Share))
        onDestroy()
        dismiss()
    }
}